...
export const getAllMenuAccessByRoleId = async (req: Request, res: Response) => {
  let err;
  ...
  let role;
  [err, role] = await toAwait(Role.findById(id));
  ...
  let getAllRoleMenu;
  [err, getAllRoleMenu] = await toAwait(RoleMenu.find({ roleId: id }).populate("menuId"))

  if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);

  getAllRoleMenu = getAllRoleMenu as IRoleMenu[]
  if (getAllRoleMenu.length === 0) {
    return ReE(res, { message: `role menu not found for this role!.` }, httpStatus.NOT_FOUND)
  }

  let data = {
    role: role,
    menus: getAllRoleMenu
  }

  ReS(res, { message: "Role menu found successfully", data: data }, httpStatus.OK)
